...
def minmax_scale_dataset(train, val, test):
    # Normalize data
    min_val = torch.min(torch.cat([d.y for d in train + val + test]))
    max_val = torch.max(torch.cat([d.y for d in train + val + test]))
    for data in train + val + test:
        data.y = (data.y - min_val) / (max_val - min_val)
    return train, val, test, min_val, max_val

def reverse_minmax_scale_test(test_pred, test_true, min_val, max_val):
    # Apply reverse min-max scaling to each tensor in the lists
    test_pred = [pred * (max_val - min_val) + min_val for pred in test_pred]
    test_true = [true * (max_val - min_val) + min_val for true in test_true]
...
def run_model(model_type, num_samples, config, train, val, test, min_val, max_val):
    ...
    # Descale
    pred_list, true_list = reverse_minmax_scale_test(pred_list, true_list, min_val, max_val)
    
    # Calculate R2 score
    true_list_cpu = [t.cpu().numpy() for t in true_list]
    pred_list_cpu = [t.detach().cpu().numpy()[0] for t in pred_list]
    R2 = r2_score(true_list_cpu, pred_list_cpu)
    
    return test_MAE / len(test), test_MSE / len(test), R2
...
def run_experiment():
    ...
    filename = os.path.join(dirpwd, "qm9.json")
    ...
    for repeat in range(num_repeats):
        ...
        # Scale Dataset
        train, val, test, min_val, max_val = minmax_scale_dataset(train, val, test)
        # Run dataset for all models
        for model_type in model_types:
            print(f"\nRunning model: {model_type}")
            MAE, MSE, R2 = run_model(model_type, num_sample, config, train, val, test, min_val, max_val)
    ...
    output_file = "results_rank_use_edge_lengths.json"
