...
class PAINNStack(Base):
    ...
    def _conv_args(self, data):
        ...
        # Assuming data.supercell_size is a 3x3 tensor representing the simulation cell
        ...
        diff = self.get_distance_vectors(data.pos[i], data.pos[j], supercell_size)  # Shape: (num_edges, 3)

        # Compute distances
        dist = diff.pow(2).sum(dim=-1).sqrt()  # Shape: (num_edges,)

        # Normalize distance vectors
        # Avoid division by zero by adding a small epsilon to dist
        epsilon = 1e-8
        norm_diff = diff / (dist.unsqueeze(-1) + epsilon)  # Shape: (num_edges, 3)
